...
function updatePersonsSpinner(list) {
 console.log("calls");
 var min = 0;
 var max = 0;
 var count = 0;
 for (var ind in tcanvas_.sidsIncluded) {
  if(tcanvas_.sidsIncluded[ind]==1) {
   var sid = ind;
   for(var f=0;f < floorCanvases.length;f++) {
     for(var s= 0 ; s < floorCanvases[f].shapes.length;s++) {
	   if(floorCanvases[f].shapes[s].sid == sid) {
	      min+=floorCanvases[f].shapes[s].booking_options.minPersons;
		  max+=floorCanvases[f].shapes[s].booking_options.maxPersons;
		  break;
	   }
	 }
  }
     ...
     for(var s= 0 ; s < floorCanvases[f].listSelected.length;s++) {
	   if(floorCanvases[f].listSelected[s].sid == sid) {
		  ...
		  if(floorCanvases[f].listSelected.length == 0) {
			...
			updatePersonsSpinner(floorCanvases[f].listSelected);
		}
	 }
  }
}
