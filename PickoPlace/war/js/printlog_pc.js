...
function createSaveObjectPre() {
	...
	$("#canvas_drawall_images").remove();
	var appendData = '<div id="canvas_drawall_images" style="display:none;"></div>';
	$("#canvas_drawall_images_wrap").append(appendData);
    for (var i = 0 ; i < floorCanvases.length ; i ++) {
	  var state = floorCanvases[i];
	  // Create image from all canvas
	  state.selection = null;
	  state.temp_list_selected = state.listSelected;
	  state.listSelected = [];
	  state.drawAll = true;
	  zoomReset(state);
  }	  
