...
function ApplySelectors() {
		// Update iframe selectors
		for(var f = 0 ; f < floorCanvases.length ; f++ ) {
		  var floor = floorCanvases[f];
		  
		if (floor.mainfloor) {

				var appendData = '<div class="floor_on_top_single"><input type="radio" class="floor_top_button" id="floor_single_button-'+floor.floorid+'" name="floor_single_" value="'+floor.floorid+'" checked="checked"><div class="floor_name_inb">'+floor.floor_name+'</div></div>';
		        $("#floors_single_radio_form").prepend(appendData);
				var appendData = '<div class="floor_dimens_sel"><input type="radio" class="floor_top_button_dim" id="floor_single_dimensions-'+floor.floorid+'" name="floor_single_dim" value="'+floor.floorid+'" checked="checked"><div class="floor_name_inb">'+floor.floor_name+'</div></div>';
		        $("#floors_dimensions_selector_div").prepend(appendData);		
                floorSelectedIDDim = floor.floorid;			
	    	 } else {
                var appendData = '<div class="floor_on_top_single"><input type="radio" class="floor_top_button" id="floor_single_button-'+floor.floorid+'" name="floor_single_" value="'+floor.floorid+'" ><div class="floor_name_inb">'+floor.floor_name+'</div></div>';
		        $("#floors_single_radio_form").append(appendData);
				var appendData = '<div class="floor_dimens_sel"><input type="radio" class="floor_top_button_dim" id="floor_single_dimensions-'+floor.floorid+'" name="floor_single_dim" value="'+floor.floorid+'" ><div class="floor_name_inb">'+floor.floor_name+'</div></div>';
		        $("#floors_dimensions_selector_div").append(appendData);				
	    	 }
		}
	    ...
	    var iframeWidth = document.getElementById("iframe_wrap").offsetWidth;
		 ...
		 $("#iframe_set_width").val(iframeWidth);
		 $('#iframe_width').slider('value', iframeWidth);
		 $("#iframe_set_height").val(iframeHeight);
		 $('#iframe_height').slider('value', iframeHeight);
}
...
$('#iframe_width').slider('value', iframe.iw);
$("#iframe_wrap").css("width",iframe.iw);
$('#iframe_height').slider('value', iframe.ih);
$("#iframe_wrap").css("height",iframe.ih);
 ...
 if(iframe.floors == "both") {
    ...
    for(var f = 0 ; f < iframe.dimentions.length ; f++ ) {
	   w = iframe.dimentions[f].w;
	   h = iframe.dimentions[f].h;
	}
 ...
 if(iframe.book == true) {
   ...
   $("#feo_booking_available_checkbox").click();
   
   if(iframe.booktop==true) {
      $('#book_frame_ontop-yes').prop('checked', true);
	  $('#book_frame_ontop-no').prop('checked', false);
	  $("#book_frame_ontop-yes").click();
   } else {
      $('#book_frame_ontop-yes').prop('checked', false);
	  $('#book_frame_ontop-no').prop('checked', true);  
	  $("#book_frame_ontop-no").click();
   }
   if(iframe.bookfill == true) {
      $('#book_frame_wraps-yes').prop('checked', true);
	  $('#book_frame_wraps-no').prop('checked', false);
	  $('#book_frame_wraps-yes').click();
	  $('#book_frame_wraps-yes').click();
   } else {
      $('#book_frame_wraps-yes').prop('checked', false);
	  $('#book_frame_wraps-no').prop('checked', true);
	  $('#book_frame_wraps-no').click();   
	  $('#book_frame_wraps-no').click();
   }
 } else {
 ...
 var color = iframe.color.replace(/^\#/, "");;
 $('#selected_fill_color').colpickSetColor(color,false);
}
