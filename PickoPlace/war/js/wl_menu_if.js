...
// Border height	
	...
	function floorAppend(appendTo_,singleBoth,singleFloorID,temp) {
	   ...
	   if(singleBoth) {
			...
			appendData =' <div id="canvas_wrap_not_scroll_conf-'+singleFloorID+'" class="canvas_wrap_not_scroll_if"></div>';
			$("#canvas_appended_wrapper-"+singleFloorID).append(appendData);
		...
		if(floorCanvases.length > 1) {	
			...
			appendData =' <div id="canvas_wrap_not_scroll_conf-both" class="canvas_wrap_not_scroll_if"></div>';
			$("#canvas_appended_wrapper-both").append(appendData);
			for (var f = 0 ;f < floorCanvases.length ; f++) {              
	             canvas_ref = floorCanvases[f];
	...
	function ApplyInitialPositionFE() {
	  ...
	  for (f= 0 ; f < floorCanvases.length ; f++) {
	    ...
	    document.getElementById("iframe_wrap").style.width = browserWidth  + "px";
	    document.getElementById("iframe_wrap").style.height = browserHeight  + "px";
	...
	function organizeViews (top_perc , bot_perc ,right_width,topleft_perc , topright_perc , botleft_perc , botright_perc , idtleft , idtright , idbleft , idbright ) {
	  ...
	  console.log(document.getElementById("content_top_row").style.height);
...
$("#iframe_wrap").css("height",iframe.ih);
 ...
 if(iframe.book == true) {
   bookAvailable(true);
   bookOnTop(iframe.booktop);
   bookFill(iframe.bookfill);
 } else {
   bookAvailable(false);
 }
...
function bookOnTop(bool) {
	console.log("BOOKTOP:"+bool);
  if(bool) {
