...
function Shape(state, x, y, w, h, fill,type) {
  "use strict";
  ...
  printLog('logs_window','Shape added('+this+'):width='+w+',height='+h+',x='+x+',y='+y,'blue');
...
Shape.prototype.draw = function(ctx, optionalColor) {
  ...
  } else if (this.type_ == "image" ) {
    //ctx.fillRect(this.x, this.y, this.w, this.h);
	var img_=  document.getElementById("image_");
	ctx.drawImage(img_,this.x,this.y,this.w,this.h);
  }
  if (this.state.selection === this) {
    ...
    this.state.selectionHandles[8].x = this.x + this.w + 10 ;
    this.state.selectionHandles[8].y = this.y  - 14;
...
function CanvasState(canvas) {
  ...
  canvas.addEventListener('dblclick', function(e) {
	...
	if(document.getElementById("myCheck").checked == true) {
	  printLog('logs_window','Image selected','green');
	  myState.addShape(new Shape(myState, mouse.x - 10, mouse.y - 10, 20, 20, 'rgba(0,255,0,.6)',"img"));
	} else {
      myState.addShape(new Shape(myState, mouse.x - 10, mouse.y - 10, 20, 20, 'rgba(0,255,0,.6)',"rect"));
	}
  }, true);
...
CanvasState.prototype.addShape = function(shape) {
  ...
  printLog('logs_window','Shapes count = '+this.shapes.length,'red');
  this.valid = false;
};
...
function init() {
  ...
  canvas_.addShape(new Shape(canvas_, 260, 70, 60, 65, 'rgba(0,205,0,0.7)',"rect"));
  // add a green-blue rectangle
  canvas_.addShape(new Shape(canvas_, 240, 120, 40, 40, 'rgba(2,165,165,0.7)',"rect"));  
  // add a smaller purple rectangle
  canvas_.addShape(new Shape(canvas_, 5, 60, 25, 25, 'rgba(150,150,250,0.7)',"rect"));
}

function dbDrawRect(ctx,x,y,w,h,strokeColor,fillColor,alpha,sw) {
  ctx.lineWidth = sw;
...
function dbRoundRect (ctx,x,y,w,h,strokeColor,fillColor,alpha,sw,R) {
//
...
function dbCircle (ctx , x, y, radius, startAngle, endAngle,strokeColor,fillColor,alpha,sw) {
  ctx.lineWidth = sw;
  ctx.strokeStyle = strokeColor;
  ctx.fillStyle = fillColor;
    if (sw == 0) {
...
function dbTrapez (ctx,x,y,w,h,strokeColor,fillColor,alpha,sw,cutx) {
//
...
//
  ...
  var cutX = w / 2 * cutx  / 100; // cutx in percentage
    ...
    ctx.beginPath();
    ctx.moveTo(cx+cutX,y);
    ctx.lineTo(cx+w-cutX,y);
    ctx.lineTo(cx+w,cy+h);
    ctx.lineTo(cx,cy+h);
    ctx.lineTo(cx+cutX,y);
	ctx.closePath();
  ctx.globalAlpha = alpha;
  ctx.fill();
  ctx.stroke();
