...
function updateBookingSidPopover(popover_hidden_wrap_id, sid_shape) {
    ...
    if (document.getElementById("bookingavailable") == null) {
        appendData += '		<div class="row iframe_place_popover_content"   data-toggle="tooltip"   data-placement="top" title="מספר אנשים">';
        appendData += '			<div class="col-xs-6 iframe_popover_val">';


            if (sid_shape.booking_options.minPersons == sid_shape.booking_options.maxPersons) {
                appendData += '			   ' + sid_shape.booking_options.minPersons;
            } else {
                appendData += '			   ' + sid_shape.booking_options.minPersons + '-' + sid_shape.booking_options.maxPersons + '';
            }
        appendData += '			</div> ';
        ...
        if (sid_shape.isAvailable == false) {
            if (sid_shape.placeClosed == true) {
                appendData += ' <div id="lonitan_text">מקום סגור</div>';
            } else if (sid_shape.ordered == true) {
                appendData += ' <div id="lonitan_text">מקום שמור</div>';
            } else if (sid_shape.booking_options.minPersons < parseInt($("#book_persons_val_").val()) ||
                sid_shape.booking_options.maxPersons > parseInt($("#book_persons_val_").val())) {
                appendData += ' <div id="lonitan_text">לא תואם כמות אנשים</div>';
            }
